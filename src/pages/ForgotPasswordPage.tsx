...
export const ForgotPasswordPage: React.FC<ForgotPasswordPageProps> = ({ onNavigate, language }) => {
  const tr = t(language);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
      <div className="max-w-md mx-auto bg-white rounded-3xl border border-slate-200/90 shadow-xl p-6 sm:p-8 text-center">
        ...
        <h1 className="text-xl sm:text-2xl font-extrabold text-slate-900 mb-2">{tr('forgot.title')}</h1>
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-left">
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">{tr('forgot.emailLabel')}</label>
              <input
                type="email"
                required
                value={email}
