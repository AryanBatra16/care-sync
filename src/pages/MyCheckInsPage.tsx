  ...
  return (
    <div className="relative">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10">
        {/* Top Banner */}
        <div className="glass rounded-3xl p-6 sm:p-8 shadow-soft mb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          ...
          >
              ...
              {MOODS.map((m) => (
                <button
                  ...
                  className={`aspect-square rounded-2xl flex flex-col items-center justify-center gap-1 border-2 transition-all cursor-pointer ${
                    selectedMood === m.type ? `${m.tint} ${m.ring} scale-105 shadow-xs` : 'border-transparent bg-white/40 hover:bg-white/60'
                  }`}
                >
            ...
            <textarea
              ...
              rows={2}
              className="w-full rounded-2xl border border-white/60 bg-white/60 p-3 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#005c55] resize-none mb-3"
            />
            ...
            {savedNotice && (
              <div className="mt-3 p-3 rounded-xl bg-emerald-50/90 border border-emerald-200 text-emerald-800 text-xs font-bold flex items-center gap-2">
          ...
          <div className="flex flex-col gap-4">
            <motion.div
              initial={{ opacity: 0, y: 12 }}
              animate={{ opacity: 1, y: 0 }}
              className="glass rounded-3xl p-5 shadow-soft flex flex-col items-center text-center"
            >
              ...
              <div className="grid grid-cols-7 gap-1">
                {heatmapDays.map((d, i) => (
                  <div
                    ...
                    style={{ background: d.mood ? HEAT_COLOR[d.mood] : 'rgba(148,163,184,0.15)' }}
                  />
                ))}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 flex flex-col gap-4">
            {/* Filters and count */}
            <div className="flex items-center justify-between glass p-4 rounded-2xl shadow-soft">
              ...
              <div className="space-y-3">
                {filtered.map((item) => (
                  <motion.div
                    ...
                    className="glass rounded-2xl p-5 shadow-soft hover:shadow-glow transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 group"
                  >
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-white/60 flex items-center justify-center text-2xl shrink-0">
                        {item.emoji}
                      </div>
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold bg-white/70 text-slate-700">
                            {item.mood}
                          </span>
                        </div>
                    ...
                    <button
                      ...
                      className="self-end sm:self-center px-3.5 py-1.5 rounded-xl bg-white/70 hover:bg-white text-teal-800 border border-white/60 text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer shrink-0"
                    >
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            ...
            {/* Data Purge Vault */}
            <div className="rounded-3xl p-5 border border-rose-200/60" style={{ background: 'rgba(255, 228, 230, 0.55)', backdropFilter: 'blur(20px)' }}>
