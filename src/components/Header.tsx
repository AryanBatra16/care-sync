    ...
    <>
      ...
      <header
        className={`fixed top-0 left-0 right-0 z-40 backdrop-blur-xl border-b transition-all ${
          isLandingPage
            ? 'bg-white/80 border-slate-200/60 shadow-none'
            : 'bg-white/90 border-slate-200/80 shadow-[0_1px_8px_rgba(0,0,0,0.03)]'
        }`}
      >
        {isLandingPage ? (
          /* Public Landing Nav: Logo — centered links — Sign In / Sign Up */
          <div className="h-16 md:h-20 w-full px-4 sm:px-6 lg:px-10 max-w-7xl mx-auto flex items-center justify-between gap-4">
            <button
      ...
      {/* Backdrop + Sidebar Drawer (hidden on the public landing page) */}
      {!isLandingPage && (
        <>
          ...
          >
            ...
            {/* Drawer Bottom Bar: Crisis Support */}
